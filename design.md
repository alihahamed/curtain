...
- After the grid, in order: an ask in place of testimonials (no card, just a large "Your words could sit right here." over a drifting wall of question-mark tiles at 35% opacity, inert and faded on every edge, a button to post on X at @AhmedAli8177), the tools it is built on as plain wordmarks between dashed rules, Devouring Details by Rauno Freiberg as the named inspiration with two link rows, then the FAQ. Each rises 48px as it comes in, once, with no fade.
