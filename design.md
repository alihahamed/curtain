...
- After the grid, in order: an ask in place of testimonials (a dashed card, "Your words could sit right here.", a button to post on X at @AhmedAli8177), the tools it is built on as plain wordmarks between dashed rules, Devouring Details by Rauno Freiberg as the named inspiration with two link rows, then the FAQ. Each rises 48px as it comes in, once, with no fade.
